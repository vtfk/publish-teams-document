...
const { modifyColumn, getColumns, getViews, addView, removeView, removeViewField, addViewField, upsertView, cleanUpDefaultView, getList } = require('./sharepoint-requests')
...
const createSharepointClient = (authConfig) => {
  const client = {
    ...
    upsertView: async (libraryUrl, listId, view) => { return await upsertView(authConfig, libraryUrl, listId, view) },
    cleanUpDefaultView: async (libraryUrl, listId, removeFields) => { return await cleanUpDefaultView(authConfig, libraryUrl, listId, removeFields) },
