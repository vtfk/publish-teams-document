...
const getDriveItemData = async (documentData) => {
  ...
  writeFileSync('./ignore/driveItemVersions.json', JSON.stringify(driveItemVersions, null, 2))
  ...
  const versionLastModifiedByUser = driveItemVersions.value.find(version => version.lastModifiedBy?.user?.email) // In case of lastModified by sharepoint app, hmm maybe sort on versioins
  if (!versionLastModifiedByUser) {
    throw new Error('Document has never been modified by a user, whaaat?') 
  }
  const publisher = versionLastModifiedByUser.lastModifiedBy.user
