...
const getDriveItemData = async (documentData) => {
  ...
  const actualLatestVersionNumber = driveItemVersions.value.map(driveItemVersion => Number(driveItemVersion.id)).sort().pop() // Støgg måte, henter ut alle versjonsnummer som tall, sorterer stigende, og bøffer ut siste element
  ...
  logger('info', `It was selveste ${publisher.email} that last modified the version we are going to publish`)
