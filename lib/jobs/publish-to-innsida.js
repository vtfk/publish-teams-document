...
const publishToInnsida = async (documentData) => {
  ...
  const metadata = {
    ...
    innsida_weburl: uploadResult.response.webUrl,
  ...
  logger('info', ['publishToInnsida', 'Successfully set metadata on the uploaded document'])

  logger('info', ['publishToInnsida', 'Finished job getDriveItemData'])
  return { uploadResult, updateFieldsResult }
}
