...
const setupSourcePublishView = (sourceLibraryConfig) => {
  ...
  // Add columnns in order you like it to be displayed
  publishView.columns.push(COLUMN_NAMES_PUBLISHING_CHOICES_NAME) // Kolonnen for hvor dokumentet publiseres
  if (sourceLibraryConfig.hasDocumentResponsible) {
    publishView.columns.push(COLUMN_NAMES_DOCUMENT_RESPONSIBLE_NAME) // Kolonnen for hvem som er dokumentansvarlig
  } else {
    publishView.removeColumnsIfExists.push(COLUMN_NAMES_DOCUMENT_RESPONSIBLE_NAME) // Hvis ikke den er definert i config, fjerner vi den hvis den finnes
  }
  if (sourceLibraryConfig.innsidaPublishing) {
    publishView.columns.push(COLUMN_NAMES_PUBLISHED_SHAREPOINT_URL_NAME) // Kolonnen for innsidaUrl
  } else {
    publishView.removeColumnsIfExists.push(COLUMN_NAMES_PUBLISHED_SHAREPOINT_URL_NAME) // Hvis ikke den er definert i config, fjerner vi den hvis den finnes
  }
  if (sourceLibraryConfig.webPublishing) {
    publishView.columns.push(COLUMN_NAMES_PUBLISHED_WEB_URL_NAME) // Kolonnen for nettsideurl
  } else {
    publishView.removeColumnsIfExists.push(COLUMN_NAMES_PUBLISHED_WEB_URL_NAME) // Hvis ikke den er definert i config, fjerner vi den hvis den finnes
  }
  publishView.columns.push(COLUMN_NAMES_PUBLISHING_CHOICES_NAME) // Kolonnen for hvem som publiserte sist
  publishView.columns.push('Modified')
  publishView.columns.push('Editor') // Kolonnen for hvem som redigerte dokumentet sist
  publishView.columns.push('_UIVersionString') // Kolonnen for versjon
  publishView.columns.push(COLUMN_NAMES_PUBLISHED_VERSION_NAME) // Kolonnen for versjon
