...
const handleFailedJob = async (jobName, documentData, error) => {
  ...
  let errorMsg = error.response?.data || error.stack || error.toString()
  try {
  ...
  if (documentData.flowStatus.runs >= retryIntervalMinutes.length) {
    try {
      ...
      renameSync(documentData.flowStatus.documentPath, `./documents/error/${documentData.flowStatus.documentName}.json`)
    } catch (error) {
...
const finishDocument = (documentData) => {
  ...
  // temp do not delete if (existsSync(getDriveItemDataResult.cachedFile.path)) unlinkSync(getDriveItemDataResult.cachedFile.path)
