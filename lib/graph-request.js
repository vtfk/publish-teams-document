...
const { sourceGraphClient: { baseurl } } = require('../config')
 ...
 * @param {boolean} [options.tenant] - If you want to use source (default) or destination tenant
...
const singleGraphRequest = async (resource, options = { method: 'get', body: {} }) => {
  ...
  const tokenOptions = options.tenant ? { tenant: options.tenant } : undefined
  const token = await getGraphToken(tokenOptions)
