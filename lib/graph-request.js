 ...
 * @param {boolean} [options.method] - If you want to use another method than GET
 * @param {boolean} [options.beta] - If you want to use the beta api
...
const singleGraphRequest = async (resource, options = { method: 'get', body: {} }) => {
  ...
  const { queryParams, beta, advanced, body, method } = options ?? {}
  const token = await getGraphToken()
  logger('info', ['singleGraphRequest', 'calling graph', method])
  ...
  logger('info', ['singleGraphRequest', 'got data from graph'])
  return data
}
